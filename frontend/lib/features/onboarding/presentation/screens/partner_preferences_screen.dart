...
const _incomeOptions = ['INR below 1 Lakh', 'INR 1–5 Lakh', 'INR 5–10 Lakh', 'INR 10–20 Lakh', 'Open to All'];
const _managedByOptions = ['Self', 'Parent', 'Sibling', 'Relative', 'Friend', 'Open to All'];
const _dietOptions = ['Veg', 'Non-Veg', 'Eggetarian', 'Jain', 'Vegan', 'Open to All'];
...
class PartnerPreferencesScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PartnerPreferencesScreen> createState() => _PartnerPreferencesScreenState();
}

class _PartnerPreferencesScreenState extends ConsumerState<PartnerPreferencesScreen> {
  ...
  void _seedIfNeeded() {
    ...
    if (prefs.ageMin != null) return; // already has values (e.g. coming back to this screen)
    final age = draft.age ?? 25;
    final height = draft.heightCm ?? 165;
    ref.read(profileCreationControllerProvider.notifier).updatePartnerPreferences((_) =>
        PartnerPreferences(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('${formatValue(values.start)} – ${formatValue(values.end)}',
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _PrefCard(title: 'Basic Details', color: context.colors.success, rows: [
                    _PrefRow(
                      icon: Icons.calendar_today_rounded,
                      label: 'Age Range',
                      value: '${prefs.ageMin ?? 21} to ${prefs.ageMax ?? 35}',
                      onTap: () => _editRange(
                        title: 'Age Range',
                        min: 18,
                        max: 65,
                        ...
                        onSave: (s, e) => updatePrefs(
                            (p) => p.copyWith(ageMin: s.round(), ageMax: e.round())),
                      ),
                    ),
                    _PrefRow(
                      icon: Icons.height_rounded,
                      label: 'Height Range',
                      value: '${_heightLabel(prefs.heightMinCm)} to ${_heightLabel(prefs.heightMaxCm)}',
                      onTap: () => _editRange(
                        title: 'Height Range',
                        min: 122,
                        max: 213,
                        ...
                        onSave: (s, e) => updatePrefs((p) =>
                            p.copyWith(heightMinCm: s.round(), heightMaxCm: e.round())),
                      ),
                    ),
                    _PrefRow(
                      icon: Icons.groups_rounded,
                      label: 'Marital Status',
                      value: prefs.maritalStatuses.isEmpty ? 'Open to All' : prefs.maritalStatuses.join(', '),
                      onTap: () => _editSingle(
                        title: 'Marital Status',
                        options: _maritalOptions,
                        value: prefs.maritalStatuses.isEmpty ? null : prefs.maritalStatuses.first,
                        onSave: (v) => updatePrefs((p) => p.copyWith(
                            maritalStatuses: v == 'Open to All' ? [] : [v])),
                      ),
                    ),
                  ]),
                  _PrefCard(title: 'Community', color: context.colors.warning, rows: [
                    _PrefRow(
                    ...
                    _PrefRow(
                      ...
                      label: 'Mother Tongue',
                      value: prefs.motherTongues.isEmpty ? 'Open to All' : prefs.motherTongues.join(', '),
                      onTap: () => _editSingle(
                        title: 'Mother Tongue',
                        options: _motherTongueOptions,
                        value: prefs.motherTongues.isEmpty ? null : prefs.motherTongues.first,
                        onSave: (v) => updatePrefs(
                            (p) => p.copyWith(motherTongues: v == 'Open to All' ? [] : [v])),
                      ),
                    ),
                  ]),
                  if (_expanded) ...[
                    _PrefCard(title: 'Location', color: context.colors.accent, rows: [
                      _PrefRow(
                    ...
                    _PrefCard(title: 'Education & Career', color: context.colors.gold, rows: [
                      _PrefRow(
                        ...
                        onTap: () => _editSingle(
                          ...
                          onSave: (v) => updatePrefs((p) =>
                              p.copyWith(highestQualification: v == 'Open to All' ? null : v)),
                        ),
                      ),
                      _PrefRow(
                        ...
                        onTap: () => _editSingle(
                          ...
                          onSave: (v) => updatePrefs(
                              (p) => p.copyWith(workingWith: v == 'Open to All' ? null : v)),
                        ),
                      ),
                      _PrefRow(
                        ...
                        onTap: () => _editSingle(
                          ...
                          onSave: (v) => updatePrefs(
                              (p) => p.copyWith(profession: v == 'Open to All' ? null : v)),
                        ),
                      ),
                      _PrefRow(
                        ...
                        onTap: () => _editSingle(
                          ...
                          onSave: (v) => updatePrefs(
                              (p) => p.copyWith(incomeMin: v == 'Open to All' ? null : v)),
                        ),
                      ),
                    ]),
                    _PrefCard(title: 'Other Details', color: context.colors.muted, rows: [
                      _PrefRow(
                        ...
                        onTap: () => _editSingle(
                          ...
                          onSave: (v) => updatePrefs((p) =>
                              p.copyWith(profileManagedBy: v == 'Open to All' ? null : v)),
                        ),
                      ),
                      _PrefRow(
                        ...
                        onTap: () => _editSingle(
                          ...
                          value: prefs.diet,
                          onSave: (v) =>
                              updatePrefs((p) => p.copyWith(diet: v == 'Open to All' ? null : v)),
                        ),
                      ),
                    ]),
                  ],
...
class _PrefCard extends StatelessWidget {
  ...
  const _PrefCard({required this.title, required this.color, required this.rows});
...
class _PrefRow extends StatelessWidget {
  ...
  const _PrefRow({required this.icon, required this.label, required this.value, required this.onTap});
