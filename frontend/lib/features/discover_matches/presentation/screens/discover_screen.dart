...
/// Swipe-through discovery deck — right/drag-right or the heart button to
/// express interest, left/drag-left or the X to skip. Screen 30 in the
/// build plan ("match card, swipe").
        ...
        child: state.loading
            ...
            : state.isExhausted
                ? EmptyState(
                    ...
                    message:
                        'Check back later for more matches, or start over.',
                    actionLabel: 'Start over',
                    onAction: () =>
                        ref.read(discoverControllerProvider.notifier).reset(),
                  )
                : Padding(
                          ...
                          child: Stack(
                            ...
                            children: [
                              if (state.next != null)
                                Transform.scale(
                                  scale: 0.95,
                                  child: _StaticCard(profile: state.next!),
                                ),
                              if (state.current != null)
                                // The X/heart buttons advance the deck
                                // without ever going through _SwipeCard's
                                // own drag-dismiss animation, so without
                                // this the next card would just appear
                                // instantly with no transition at all —
                                // AnimatedSwitcher gives it the same
                                // pop-in-from-behind feel as a real swipe.
                                AnimatedSwitcher(
...
class _SwipeCardState extends State<_SwipeCard>
  ...
  // True only while a finger is actively moving the card — the transform
  // must track the finger 1:1 with no animation lag during that. Once the
  // finger lifts (snap-back OR dismiss), this flips to false so the
  // AnimatedContainer below actually animates the transition instead of
  // teleporting; previously the snap-back case used Duration.zero always,
  // so releasing a drag that didn't cross the swipe threshold made the
  // card jump back to center instantly instead of springing back.
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    if (_drag.dx.abs() > threshold) {
      ...
      setState(() {
        _drag = Offset(liked ? 600 : -600, _drag.dy);
      });
      Future.delayed(
          const Duration(milliseconds: 180), () => widget.onSwiped(liked));
    } else {
          ...
          children: [
            ...
            Positioned(
              ...
              child: Opacity(
                opacity: likeOpacity,
                child:
                    _StampLabel(label: 'INTERESTED', color: context.colors.success),
              ),
            ),
            Positioned(
              top: 24,
              ...
              child: Opacity(
                opacity: nopeOpacity,
                child:
                    _StampLabel(label: 'SKIP', color: context.colors.danger),
              ),
            ),
          ],
        ),
