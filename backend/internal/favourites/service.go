...
var (
	ErrSelfFavourite = errors.New("cannot favourite your own profile")
...
func (s *Service) Add(ctx context.Context, userID, targetProfileID string) (Response, error) {
	...
	if targetProfile.UserID == userID {
		return Response{}, ErrSelfFavourite
	}
	...
	f, err := s.repo.Create(ctx, userID, targetProfile.UserID)
	if err != nil {
		return Response{}, err
	}
	return Response{ID: f.ID, UserID: f.UserID, TargetUserID: f.TargetUserID, CreatedAt: f.CreatedAt.Format(time.RFC3339)}, nil
}
