...
func (h *Handler) Remove(c *gin.Context) {
	...
	response.OK(c, gin.H{"message": "removed from shortlist"})
}
...
func writeServiceError(c *gin.Context, err error) {
	switch {
	case errors.Is(err, ErrSelfShortlist):
		response.Fail(c, http.StatusBadRequest, "self_shortlist", err.Error(), nil)
